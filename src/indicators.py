...
class SMA(Indicator):
    ...
    def compute(self, raw_data: pd.DataFrame) -> pd.Series:
        ...
        if len(raw_data) < self.period:
            raise ValueError(f"Not enough data points. Need at least {self.period}, got {len(raw_data)}")
...
class RSI(Indicator):
    ...
    def compute(self, raw_data: pd.DataFrame) -> pd.Series:
        ...
        if len(raw_data) < self.period + 1:
            raise ValueError(f"Not enough data points. Need at least {self.period + 1}, got {len(raw_data)}")
...
class MACDSignal(Indicator):
    ...
    def compute(self, raw_data: pd.DataFrame) -> pd.Series:
        ...
        if len(raw_data) < self.long_period + self.signal_period:
            raise ValueError(f"Not enough data points for MACD Signal calculation")
...
class MACDHistogram(Indicator):
    ...
    def compute(self, raw_data: pd.DataFrame) -> pd.Series:
        ...
        if len(raw_data) < self.long_period + self.signal_period:
            raise ValueError(f"Not enough data points for MACD Histogram calculation")
