...
class MovingAverageCross(Strategy):
    ...
    def calculate_signals(self, market_data: MarketData) -> Dict[str, pd.Series]:
        ...
        aligned_lower, aligned_upper = lower_values.align(upper_values, join='inner')
        
        bullish_cross = (aligned_lower > aligned_upper) & (aligned_lower.shift(1) <= aligned_upper.shift(1))
        
        bearish_cross = (aligned_lower < aligned_upper) & (aligned_lower.shift(1) >= aligned_upper.shift(1))
...
class RSICross(Strategy):
    def __init__(self, rsi_period: int, lower_bound: float, upper_bound: float):
        ...
        if not (0 < lower_bound < upper_bound < 100):
            raise ValueError("Bounds must be: 0 < lower_bound < upper_bound < 100")
    ...
    def calculate_signals(self, market_data) -> Dict[str, pd.Series]:
        ...
        buy_signals = (rsi_values > self.lower_bound) & (rsi_values.shift(1) <= self.lower_bound)
        
        # RSI crosses below upper bound - totally a sell!
        sell_signals = (rsi_values < self.upper_bound) & (rsi_values.shift(1) >= self.upper_bound)
...
class RSIExtremes(Strategy):
    def __init__(self, rsi_period: int, oversold_threshold: float = 30, overbought_threshold: float = 70):
        ...
        if not (0 < oversold_threshold < overbought_threshold < 100):
            raise ValueError("Thresholds must be: 0 < oversold < overbought < 100")
...
class MACDCross(Strategy):
    ...
    def calculate_signals(self, market_data) -> Dict[str, pd.Series]:
        ...
        aligned_macd, aligned_signal = macd_line.align(signal_line, join='inner')
        
        # Bullish: MACD line crosses above signal line - buy!
        buy_signals = (aligned_macd > aligned_signal) & (aligned_macd.shift(1) <= aligned_signal.shift(1))
        
        # Bearish: MACD line crosses below signal line - sell!
        sell_signals = (aligned_macd < aligned_signal) & (aligned_macd.shift(1) >= aligned_signal.shift(1))
...
class MACDHistogramStrategy(Strategy):
    def __init__(self, short_period: int = 12, long_period: int = 26, signal_period: int = 9):
        ...
        self.macd_histogram = MACDHistogram(short_period, long_period, signal_period)
